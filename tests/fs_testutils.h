...
#define LIBFS_MAX_PATH 256
    ...
    static struct fs_file_iterator *_fs_iter_file(const char *path)
    {
        struct fs_file_iterator *it = fs_iter_file(path);
        assert_non_null(it);
        return it;
    }
     ...
     * @param[in] path Some null-terminated path
     * @return A pointer for iterating over the file.
     */
    struct fs_file_iterator *fs_assert_iter_file(const char *path)
    {
        return _fs_iter_file(path);
    }
...
#define fs_assert_iter_file(path) _fs_iter_file(path)
